...
### 1.3 Рекомендации (не критично)
- Чанки `index` (589 КБ) и `pages2` (472 КБ) > 500 КБ — вынести recharts/KaTeX в `manualChunks` при желании.
- В проде заменить хранение пароля открытым текстом на bcrypt (passlib) — помечено TODO.
...
- [ ] Пароли — bcrypt (заменить demo-хранение, TODO помечен)
