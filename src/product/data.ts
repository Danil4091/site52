...
import { EXAM_LINES, MAX_PRIMARY_SCORE, PART1_COUNT, PART2_MAX_SCORES } from "./examLines";
...
export const VARIANTS: ProductVariant[] = [
  { id: "v-real-2023", year: 2023, title: "Основной период · реальные задания", region: "Открытый банк ФИПИ · 20 заданий (ЕГЭ-2027)", difficulty: "средний", available: true, isReal: true },
...
/* Шкала перевода первичного балла (0–33, структура 2027) → тестовый (0–100).
   ⚠️ ЗАГЛУШКА до публикации официальной шкалы ФИПИ для ЕГЭ-2027.
   Растянута с прежней 31-балльной шкалы на 33 балла. */
...
export interface BankTopic {
  ...
  /** Максимальный первичный балл (из единой конфигурации EXAM_LINES). */
  maxScore: number;
  /** Уровень сложности 1–3 (из единой конфигурации EXAM_LINES). */
  difficulty: 1 | 2 | 3;
...
/* ────────────────────────────────────────────────────────────────────
   Банк заданий. Названия, части, баллы и сложность берутся из единой
   конфигурации EXAM_LINES (examLines.ts) — структура ЕГЭ-2027, 33 балла.
   Здесь хранится только демо-статистика успеваемости и методические
   пометки. Это гарантирует, что номера/баллы не разъедутся с моделью.
   ──────────────────────────────────────────────────────────────────── */
const LINE_STATS: Record<number, { success: number; solved: number; attempts: number; trend: "up" | "down" | "flat"; note: string }> = {
  1: { success: 95, solved: 19, attempts: 20, trend: "up", note: "Треугольники, четырёхугольники, окружность; длины, углы, площади, подобие. Начните с прямоугольных треугольников." },
  2: { success: 90, solved: 18, attempts: 20, trend: "up", note: "Координаты, сумма и разность, умножение на число, скалярное произведение, угол между векторами." },
  3: { success: 85, solved: 17, attempts: 20, trend: "flat", note: "Пространственные фигуры, углы, расстояния, объёмы, площади; связь со стереометрией и планиметрией." },
  4: { success: 75, solved: 15, attempts: 20, trend: "flat", note: "Вероятность случайного события, простейшие классические задачи: P = m/n." },
  5: { success: 80, solved: 16, attempts: 20, trend: "up", note: "Сложение и умножение вероятностей, полная вероятность, комбинаторные методы." },
  6: { success: 62, solved: 12, attempts: 19, trend: "down", note: "🆕 Распределения, матожидание, дисперсия, среднее квадратическое отклонение; равномерное, показательное, нормальное распределения." },
  7: { success: 60, solved: 12, attempts: 20, trend: "down", note: "Рациональные, иррациональные, показательные, логарифмические, тригонометрические уравнения и системы. Проверяйте ОДЗ." },
  8: { success: 70, solved: 14, attempts: 20, trend: "flat", note: "Степени, корни, логарифмы, дробно-рациональные выражения, вычисления." },
  9: { success: 55, solved: 11, attempts: 20, trend: "down", note: "Производная, геометрический смысл, касательная, экстремумы, наибольшее/наименьшее значение, исследование функции, первообразная, интеграл." },
  10: { success: 48, solved: 10, attempts: 20, trend: "down", note: "Реальная ситуация → математическая модель; выражения, уравнения, неравенства; оценка правдоподобности." },
  11: { success: 38, solved: 8, attempts: 20, trend: "down", note: "Движение, работа, проценты; составление уравнений и неравенств. Приём: таблица → уравнение → проверка." },
  12: { success: 84, solved: 17, attempts: 20, trend: "up", note: "Графики функций, зависимости, чтение графика, свойства функции. НЕ производная — она в №9." },
  13: { success: 66, solved: 13, attempts: 19, trend: "flat", note: "🆕 Практические текстовые задачи; личные и семейные финансы; финансовые модели; составление и исследование модели." },
  14: { success: 72, solved: 9, attempts: 12, trend: "up", note: "Сложные уравнения, системы; различные методы решения." },
  15: { success: 64, solved: 7, attempts: 11, trend: "flat", note: "Многогранники и тела вращения, сечения, углы, расстояния, площади, объёмы; планиметрия внутри стереометрии." },
  16: { success: 45, solved: 4, attempts: 9, trend: "flat", note: "Решение сложных неравенств и систем; различные методы." },
  17: { success: 40, solved: 3, attempts: 8, trend: "down", note: "🆕 Реальная ситуация → модель → уравнение/неравенство/система → исследование → решение → интерпретация." },
  18: { success: 68, solved: 6, attempts: 9, trend: "up", note: "Треугольники, окружности, четырёхугольники; углы, длины, площади; доказательные рассуждения." },
  19: { success: 30, solved: 2, attempts: 7, trend: "flat", note: "Уравнения/неравенства с параметром, системы, графические методы, исследование количества решений." },
  20: { success: 52, solved: 4, attempts: 8, trend: "up", note: "Целые и натуральные числа, делимость, остатки, НОД/НОК, доказательства, контрпримеры." },
};

/* Банк выводится из единой конфигурации EXAM_LINES + демо-статистики. */
export const BANK: BankTopic[] = EXAM_LINES.map((line) => {
  const st = LINE_STATS[line.number] ?? { success: 0, solved: 0, attempts: 0, trend: "flat" as const, note: line.description };
  return {
    number: line.number,
    topic: line.title,
    part: line.part,
    maxScore: line.maxPrimaryScore,
    difficulty: line.difficulty,
    ...st,
  };
});

/* Переэкспорт для удобства (единый источник правды по баллам). */
export { EXAM_LINES, MAX_PRIMARY_SCORE, PART1_COUNT, PART2_MAX_SCORES };

export const TASK_OF_DAY = {
  number: 10, topic: "Прикладная математика",
...
export const DAILY_TIPS: string[] = [
  ...
  "№14 (часть 2, уравнения): сначала отбери корни на промежутке, и только потом оформляй. Большинство потерь — на отборе.",
  ...
  "№13 (моделирование + экономика): таблица платежей по периодам решает финансовую задачу почти сама. Заводи её всегда.",
  ...
  "Параметры (№19) начинаются с рисунка. Семейство прямых + окружность — твой план Б.",
  "Ошибка в вычислениях — тоже ошибка. Проверяй арифметику так же тщательно, как метод.",
  "В №9 (наибольшее значение) не забудь сравнить значение в критической точке и на концах отрезка.",
  "Разбор одной ошибки полезнее пяти новых задач. Журнал ошибок — твой главный инструмент роста.",
  "Теория чисел (№20): пункты «а» и «б» приносят баллы почти всегда — никогда не пропускай их.",
  ...
  "🆕 №6 (случайные величины): выучи формулы матожидания M(X) и дисперсии D(X) — новая тема 2027 года.",
  "🆕 №17 (моделирование, часть 2): обязательна интерпретация результата — без неё снимут балл, даже если модель верна.",
];
...
/* Реальный вариант (ЕГЭ-2027): №1–13 (часть 1, автопроверка) + №14–20 (часть 2). */
...
export const REAL_VARIANT: VariantTask[] = [
  /* ── Часть 1 · №1–13 · краткий ответ, автопроверка ── */
  ...
  { number: 6, category: "Случайные величины и статистика", part: 1, maxScore: 1, statement: "Случайная величина X задана таблицей распределения: принимает значения 1, 2, 3 с вероятностями 0,2; 0,5; 0,3 соответственно. Найдите математическое ожидание X.", hint: "M(X) = Σ xᵢ·pᵢ.", solution: "M(X) = 1·0,2 + 2·0,5 + 3·0,3 = 0,2 + 1,0 + 0,9 = 2,2.", answer: "2.2" },
  { number: 7, category: "Уравнения", part: 1, maxScore: 1, statement: String.raw`Найдите корень уравнения \( \log_2 (x + 3) = 4 \).`, hint: "Правая часть — степень двойки.", solution: "x + 3 = 16, x = 13.", answer: "13" },
  { number: 8, category: "Преобразование выражений", part: 1, maxScore: 1, statement: String.raw`Найдите значение выражения \( (\sqrt{23} - \sqrt{15})(\sqrt{23} + \sqrt{15}) \).`, hint: "Разность квадратов.", solution: "23 − 15 = 8.", answer: "8" },
  { number: 9, category: "Начала математического анализа", part: 1, maxScore: 1, imageUrls: [TANGENT_IMG], statement: "К графику функции y = f(x) проведена касательная, проходящая через точки (0; 1) и (2; 5). Найдите значение производной в точке касания.", hint: "Производная = угловой коэффициент касательной.", solution: "f′(x₀) = (5 − 1) / (2 − 0) = 2.", answer: "2" },
  { number: 10, category: "Прикладная математика / моделирование", part: 1, maxScore: 1, statement: "Плиточнику нужно выложить пол 3 м × 4 м квадратной плиткой со стороной 50 см. Плитка продаётся упаковками по 8 штук. Какое наименьшее число упаковок нужно купить?", hint: "Сначала — сколько плиток всего.", solution: "12 м² / 0,25 м² = 48 плиток → 48/8 = 6 упаковок.", answer: "6" },
  { number: 11, category: "Текстовые задачи", part: 1, maxScore: 1, statement: "Моторная лодка прошла против течения 15 км и вернулась, затратив на обратный путь на 40 минут меньше. Найдите скорость течения (км/ч), если скорость лодки в неподвижной воде 12 км/ч.", hint: "Уравнение по времени: t = S / v.", solution: "15/(12−v) − 15/(12+v) = 2/3 → v = 3 км/ч.", answer: "3" },
  { number: 12, category: "Функции и графики", part: 1, maxScore: 1, statement: String.raw`Найдите наименьшее значение функции \( y = x^2 - 6x + 11 \).`, hint: "График — парабола с ветвями вверх; наименьшее значение достигается в вершине x₀ = −b/(2a). Это свойство функции, не производная.", solution: "Вершина параболы x₀ = 6/2 = 3. y(3) = 9 − 18 + 11 = 2.", answer: "2" },
  { number: 13, category: "Моделирование + экономика", part: 1, maxScore: 1, statement: "На счёт в банке положили 100 000 рублей под 10% годовых. Проценты начисляются ежегодно и прибавляются ко вкладу (сложные проценты). Какая сумма в рублях будет на счёте через 2 года?", hint: "S = S₀·(1 + r)ⁿ.", solution: "S = 100 000 · 1,1² = 100 000 · 1,21 = 121 000 руб.", answer: "121000" },
  /* ── Часть 2 · №14–20 · развёрнутый ответ, проверяет преподаватель ── */
  { number: 14, category: "Уравнения", part: 2, maxScore: 2, statement: String.raw`а) Решите уравнение \( 2\cos^2 x - 5\sin x + 1 = 0 \). б) Укажите корни на отрезке \( \left[ 0;\ \dfrac{3\pi}{2} \right] \).`, criteria: "1 балл — верный ответ в п. а; 2 балла — оба пункта (π/6, 5π/6).", solution: "Через cos²x = 1 − sin²x: 2sin²x + 5sinx − 3 = 0 → sinx = 1/2." },
  { number: 15, category: "Стереометрия", part: 2, maxScore: 3, statement: "В правильной четырёхугольной пирамиде сторона основания равна 4, боковое ребро равно 3. Найдите угол между боковым ребром и плоскостью основания.", criteria: "1 балл — верное построение и угол; 2 балла — верное выражение; 3 балла — arccos(2√2/3).", solution: "Половина диагонали 2√2. cos φ = 2√2/3. Ответ: arccos(2√2/3)." },
  { number: 16, category: "Неравенства", part: 2, maxScore: 2, statement: String.raw`Решите неравенство \( \log_2 (x + 3) + \log_2 (x - 1) \leqslant 5 \).`, criteria: "1 балл — верный переход; 2 балла — ответ (1; 5].", solution: "ОДЗ x > 1; x² + 2x − 35 ≤ 0 → x ∈ (1; 5]." },
  { number: 17, category: "Математическое моделирование", part: 2, maxScore: 2, statement: "В июле планируется взять кредит. Каждый январь долг возрастает на 20%, с февраля по июнь выплачивается часть долга. Найдите сумму кредита, если за 4 года выплачено 311 040 руб.", criteria: "1 балл — верная модель; 2 балла — 200 000 руб.", solution: "Геометрическая прогрессия со знаменателем 1,2 → S = 200 000 руб." },
  { number: 18, category: "Планиметрия: доказательство и вычисление", part: 2, maxScore: 3, statement: "Окружность проходит через вершины B и C треугольника ABC и пересекает стороны AB и AC в точках C₁ и B₁. Докажите, что треугольник ABC подобен треугольнику AB₁C₁.", criteria: "1 балл — доказано подобие; 3 балла — полная задача.", solution: "Вписанный четырёхугольник → ∠AB₁C₁ = ∠ABC; общий угол A." },
  { number: 19, category: "Параметры", part: 2, maxScore: 4, statement: String.raw`Найдите все значения параметра \( a \), при каждом из которых уравнение \( |x^2 - 4x + 3| = a \) имеет ровно три различных корня.`, criteria: "1 балл — анализ графика; 4 балла — a = 1.", solution: "Минимум «загнутой» части y = 1 при x = 2 → три корня при a = 1." },
  { number: 20, category: "Теория чисел", part: 2, maxScore: 4, statement: "На доске написано более 40, но менее 48 целых чисел. Среднее арифметическое равно −3, среднее положительных — 4, отрицательных — −8. а) Сколько чисел? б) Каких больше? в) Наибольшее количество положительных?", criteria: "1 балл — п. а; 2 балла — п. б; 4 балла — все пункты.", solution: "4p − 8n = −3(p + n) → 7p = 5n. Полный разбор — на занятии." },
];
